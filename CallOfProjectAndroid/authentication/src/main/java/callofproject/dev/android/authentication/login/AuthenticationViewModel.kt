...
import callofproject.dev.adroid.app.authentication.login.presentation.LoginEvent
import callofproject.dev.adroid.app.authentication.login.presentation.LoginState
...
class AuthenticationViewModel @Inject constructor(
    ...
    fun onEvent(event: LoginEvent) = when (event) {
        is LoginEvent.OnLoginButtonClick -> login(UserLoginDTO(event.username, event.password))
    ...
    fun onRegisterClick() {
        viewModelScope.launch { _uiEvent.send(UiEvent.Navigate(Route.REGISTER_PAGE)) }
    }
